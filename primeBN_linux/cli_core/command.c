...
int cmd_execute_system_command (char *command, int argc, char **argv)
{
	...
	else if (pid == 0)
	{
		...
		case 8:
			...
			break;
    case 9:
  		ret = execlp (command, command, argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], NULL);
  		break;
    case 10:
    	ret = execlp (command, command, argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], NULL);
    	break;
		}

		/* When execlp suceed, this part is not executed. */
		fprintf (stderr, "Can't execute %s: %s (%d)\n", command, strerror (errno), ret);
		exit (1);
	}
	else
	{
